...
def chat_view(request):
    ...
    end_conversation()  # Asegúrate de tener una plantilla llamada chat.html


def load_instructions():
    # Usar BASE_DIR para construir la ruta completa a los archivos
    instructions_path = os.path.join(settings.BASE_DIR, 'indications', 'instructions.txt')
    company_data_path = os.path.join(settings.BASE_DIR, 'indications', 'company_data.txt')
    ...
    try:
        ...
        print(f"Error al cargar archivos: {e}")  # Imprimir error en consola para depuración
